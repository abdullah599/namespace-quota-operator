...
var _ = Describe("Manager", Ordered, func() {
	...
	Context("Manager", func() {
		...
		It("should prevent users from updating or deleting managed resources", func() {
			...
			cmd = exec.Command("kubectl", "patch", "resourcequota",
				"dev-quota-ns-protected",
			...
			cmd = exec.Command("kubectl", "patch", "limitrange",
				"dev-limits-ns-protected",
				"--namespace", "ns-protected",
				"--type", "merge",
				"--patch", `{"spec":{"limits":[{"type":"Container","default":{"cpu":"10"}}]}}`)
			output, err = utils.Run(cmd)
			...
			cmd = exec.Command("kubectl", "delete", "resourcequota", "dev-quota-ns-protected", "--namespace", "ns-protected")
			output, err = utils.Run(cmd)
		...
		It("should remove labels, resource quotas, and limit ranges when quota profile is deleted", func() {
			...
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "resourcequota", "-n", "ns-cleanup")
				_, err := utils.Run(cmd)
				g.Expect(err).To(HaveOccurred(), "ResourceQuota should be deleted")

				cmd = exec.Command("kubectl", "get", "limitrange", "-n", "ns-cleanup")
				_, err = utils.Run(cmd)
				g.Expect(err).To(HaveOccurred(), "LimitRange should be deleted")
